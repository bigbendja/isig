// src/pages/InstitucionEditar.tsx
...
export function InstitucionEditar() {
  ...
  const [form, setForm] = useState({
    nombre: '', nombre_corto: '', tipo_entidad: '', sector: '',
    pais_registro: '', sede_ciudad: '', estado_legal: 'activa',
    web_principal: '', email_contacto: '', telefono_central: '',
    nivel_acceso_requerido: 1, fuente_primaria: 'Manual', actividad_desc: '',
  })

  useEffect(() => {
    if (inst) {
      setForm({
        nombre:             (inst as any).nombre || '',
        nombre_corto:       (inst as any).nombre_corto || '',
        tipo_entidad:       (inst as any).tipo_entidad || '',
        sector:             (inst as any).sector || '',
        pais_registro:      (inst as any).pais_registro || '',
        sede_ciudad:        (inst as any).sede_ciudad || '',
        estado_legal:       (inst as any).estado_legal || 'activa',
        web_principal:      (inst as any).web_principal || '',
        email_contacto:     (inst as any).email_contacto || '',
        telefono_central:   (inst as any).telefono_central || '',
        nivel_acceso_requerido: (inst as any).nivel_acceso_requerido || 1,
        fuente_primaria:    (inst as any).fuente_primaria || 'Manual',
        actividad_desc:     (inst as any).actividad_desc || '',
      })
      setSectores((inst as any).sector ? (inst as any).sector.split(', ').filter(Boolean) : [])
    }
  }, [inst])

  const mutation = useMutation({
    mutationFn: (data: any) => institucionesService.actualizar(id!, data),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      const msg = err?.response?.data?.detail || 'Error al actualizar la institución'
  ...
  return (
    <div className="max-w-2xl mx-auto animate-in space-y-4">
      <div className="flex items-center gap-3">
        <button className="btn text-xs py-1" onClick={() => navigate(`/instituciones/${id}`)}>
          ← Volver
        </button>
        <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-3 pb-8">
          <button type="button" className="btn text-xs py-2 px-4" onClick={() => navigate(`/instituciones/${id}`)}>
            Cancelar
          </button>
